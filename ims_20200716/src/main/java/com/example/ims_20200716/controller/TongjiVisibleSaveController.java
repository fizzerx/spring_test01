...
@RestController
public class TongjiVisibleSaveController {
    @Autowired
    JdbcTemplate jdbcTemplate;

    @RequestMapping("tongjiVisibleSave")
    public Map<String, Object> tongjiVisibleSave(@RequestBody Map<String, Object> reqParams, HttpServletRequest httpServletRequest){
        ...
        if(BEQ_BODY != null) {
            ...
            String inputLableNameOne =  BEQ_BODY.get("inputLableNameOne").toString();
            String CUST_GRP_ID = (String) BEQ_BODY.get("CUST_GRP_ID");
            String groupInfo = (String) BEQ_BODY.get("groupInfo");
            String grpAnalyStatus = (String) BEQ_BODY.get("grpAnalyStatus");
            String formaDate = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
            String formaTime = new SimpleDateFormat("HH:mm:ss").format(new Date());
            System.out.println("inputLableNameOne:"+inputLableNameOne);
            // 生成ID
            String GRP_ANAL_ID = "";
            String sql = "";
            ...
            String sql3 = "insert into IMS_TEL_MODEL_LABLE(MODEL_ID,LABLE_ID) values('"+MODEL_ID+"','"+inputLableNameOne+"')";
